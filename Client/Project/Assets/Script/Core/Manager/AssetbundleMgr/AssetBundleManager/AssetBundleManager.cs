...
{	
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        const string kSimulateAssetBundles = "SimulateAssetBundles."+AppSetting.ProjectName;
#endif
        ...
        {
            BaseDownloadingURL = absolutePath + AppSetting.PlatformName + "/";
        }
        ...
        {
            return Initialize(AppSetting.PlatformName);
        }
        ...
        static public void UnloadAssetBundle(string assetBundleName, bool isForce = false)
        {
#if UNITY_EDITOR
            ...
            UnloadAssetBundleInternal(assetBundleName, isForce);
            UnloadDependencies(assetBundleName, isForce);

            //Debug.Log(m_LoadedAssetBundles.Count + " assetbundle(s) in memory after unloading " + assetBundleName);
        }

        static protected void UnloadDependencies(string assetBundleName, bool isForce)
        {
            string[] dependencies = null;
            if (!m_Dependencies.TryGetValue(assetBundleName, out dependencies))
                return;
            ...
            foreach (var dependency in dependencies)
            {
                UnloadAssetBundleInternal(dependency, isForce);
            }

            m_Dependencies.Remove(assetBundleName);
        }

        static protected void UnloadAssetBundleInternal(string assetBundleName, bool isForce)
        {
            string error;
            ...
            if (--bundle.m_ReferencedCount == 0|| isForce)
            {
